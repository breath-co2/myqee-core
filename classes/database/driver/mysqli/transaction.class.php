 ...
 */
class Database_Driver_MySQLI_Transaction extends \Database_Transaction
{
    /**
     * 当前连接ID
     *
    ...
    /**
     * 开启事务
     * @return \Database\Driver\MySQLI\Transaction
     */
    public function start()
    {
        ...
        if ( isset(static::$transactions[$this->_connection_id]) )
        {
            # 已存在事务，则该事务为子事务
            if ( $this->_set_save_point() )
            {
                //保存事务点
                static::$transactions[$this->_connection_id][$this->id] = true;
            }
            else
            {
                $this->id = null;
        ...
        else
        {
            ...
            if ( true === $this->_query('START TRANSACTION;') )
            {
                ...
                static::$transactions[$this->_connection_id] = array($this->id => true);
            }
            else
            {
                $this->id = null;
    ...
    public function commit()
    {
        ...
        {
            # 父事务
            while ( \count(static::$transactions[$this->_connection_id])>1 )
            {
                # 还有没有提交的子事务
                \end(static::$transactions[$this->_connection_id]);
                $subid = \key(static::$transactions[$this->_connection_id]);
            ...
            if ($status) unset(static::$transactions[$this->_connection_id]);
        }
        else
        {
            # 子事务
    ...
    public function rollback()
    {
        ...
        { //父事务
            ...
            if ( $status )
            {
                unset(static::$transactions[$this->_connection_id]);
            }
        }
        else
        {
    ...
    {
        if (!$this->id) return false;
        return isset(static::$transactions[$this->_connection_id]) && \key(static::$transactions[$this->_connection_id])==$this->id;
    }
    ...
    protected function _release_save_point($id)
    {
        if ( !$this->is_root() )
        {
            ...
            {
                unset(static::$transactions[$this->_connection_id][$id]);
                return true;
            }
        }
        return false;
    ...
    {
        return isset(static::$transactions[$this->_connection_id]) && isset(static::$transactions[$this->_connection_id][$this->id]);
    }
