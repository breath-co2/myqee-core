...
class OOP_ORM_Data
{
    ...
    protected function _renew_orm_config()
    {
        ...
        if ( !isset(static::$_offset_setting[$class_name]) )
        {
            static::$_offset_setting[$class_name] = array();
            static::$_field_to_offset[$class_name] = array();
        ...
        foreach ( $class_vars as $k => $v )
        {
            ...
            if ( $kstr >= 'a' && $kstr <= 'z' )
            {
                ...
                {
                    static::$_offset_setting[$class_name][$k] = $v;
                }
                unset($this->$k);
            }
        }
        ...
        if ( $need_check_config )
        {
            ...
            (
                ...
                ,
                static::$_all_id_field_name[$class_name]
            );
        }

        # 当前ID字段
        $this->_id_field_name = static::$_all_id_field_name[$class_name];
    }
    ...
    protected function _get_offset_config($index)
    {
        ...
        if ( isset(static::$_offset_setting[$this->_class_name][$index]) )
        {
            return static::$_offset_setting[$this->_class_name][$index];
        }
        else
        {
            return null;
    ...
    {
        return (array)static::$_field_to_offset[$this->_class_name][$field_name];
    }
    ...
    protected function __orm_callback_ini_data_($data = null, $is_field_key = false)
    {
        ...
        if ($is_field_key)
        {
            ...
            if ( $id && !isset(static::$_id_field_cache_data[$this->_class_name][$id]) )
            {
                static::$_id_field_cache_data[$this->_class_name][$id] = $data;
            }
        }
    }
    ...
    public static function id_field_cache_data( $class_name , $id )
    {
        ...
        if (\is_array($id))
        {
            ...
            foreach ($id as $tmpid)
            {
                if (isset(static::$_id_field_cache_data[$class_name][$tmpid]))
                {
                    $tmp[$tmpid] = static::$_id_field_cache_data[$class_name][$tmpid];
                }
            }
            return $tmp;
        }
        ...
        {
            return static::$_id_field_cache_data[$class_name][$id];
        }
    }
}