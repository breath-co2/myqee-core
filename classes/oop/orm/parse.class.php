...
abstract class Parse
{
    ...
    public static function check_config( &$setting, &$field_to_offset, &$id_field )
    {
        ...
        foreach ( $setting as $key => &$field_config )
        {
            ...
            static::_check_fieldname($key, $field_config, $field_to_offset);
            static::_check_orm($key, $field_config);
            static::_check_data($key, $field_config);
            static::_check_object($key, $field_config);

            if ( !$id_field )
            {
                static::_check_id_field($key, $field_config, $id_field);
            }
        }
        unset($field_config);
    ...
    protected static function _check_data($key, &$field_config)
    {
        ...
        {
            \Core::debug()->error('ORM字段配置错误，data属性应该为数组');
            unset($field_config['data']);
            return;
        }
        if ( ! isset($field_config['data']['driver']) )
        {
            \Core::debug()->error('ORM字段配置错误，data属性缺少driver');
            unset($field_config['data']);
            return;
        }
        if ( isset($field_config['data']['mapping']) && ! \is_array($field_config['data']['mapping']) )
        {
            \Core::debug()->error('ORM字段配置错误，data属性的mapping应该为数组');
        ...
        {
            $field_config = static::$run($field_config);
        }
        else
        {
    ...
    public static function & key_array( &$arr, $key )
    {
        if ( strlen($key) )
        {
            ...
            $k = \array_shift($kk);
            $newkey = \implode('.', $kk);
            return static::key_array($arr[$k], $newkey);
        }
        else
        {
            return $arr;
    ...
    public static function get_orm_data_by_config(\OOP\ORM\Data $data_obj, $orm_config, $index)
    {
        ...
        {
            # 设置缓存
            static::set_orm_cache_data($data_obj , $index , $c_config , $data);
        }
    ...
    public static function set_sub_offset_data( &$data , $value , array $sub_field )
    {
        ...
        if ($sub_field)
        {
            if ( \is_object($data) )
            {
                return static::set_sub_offset_data( $data->$first_field , $value , $sub_field );
            }
            else
            {
                if ( !\is_array($data) )
