...
class utf8
{
    ...
    public static function strpos($str, $search, $offset = 0)
    {
        ...
        if ( $offset == 0 )
        {
            ...
            return isset($array[1]) ? static::strlen($array[0]) : false;
        }

        $str = static::substr($str, $offset);
        $pos = static::strpos($str, $search);
    ...
    public static function strrpos($str, $search, $offset = 0)
    {
        ...
        if ( $offset == 0 )
        {
            ...
            return isset($array[0]) ? static::strlen(\implode($search, $array)) : false;
        }

        $str = static::substr($str, $offset);
        $pos = static::strrpos($str, $search);
    ...
    public static function substr($str, $offset, $length = null)
    {
        ...
        // Impossible
        if ( $length === 0 || $offset >= $strlen || ($length < 0 && $length <= $offset - $strlen) ) return '';

        // Whole string
        if ( $offset == 0 && ($length === null || $length >= $strlen) ) return $str;
    ...
    {
        if ( static::is_ascii($str) ) return ($length === null) ? \substr_replace($str, $replacement, $offset) : \substr_replace($str, $replacement, $offset, $length);

        $length = ($length === null) ? static::strlen($str) : (int)$length;
    ...
    public static function strtolower($str)
    {
        ...
        return static::from_unicode($uni);
    }
    ...
    {
        if ( static::is_ascii($str) ) return \ucfirst($str);

        \preg_match('/^(.?)(.*)$/us', $str, $matches);

        return static::strtoupper($matches[1]) . $matches[2];
    }
    ...
    {
        if ( static::is_ascii($str) ) return \ucwords($str);
    ...
    {
        if ( static::is_ascii($str1) && static::is_ascii($str2) ) return \strcasecmp($str1, $str2);
    ...
    {
        if ( static::is_ascii($search) && static::is_ascii($replace) && static::is_ascii($str) ) return \str_ireplace($search, $replace, $str, $count);

        if ( \is_array($str) )
        {
            foreach ( $str as $key => $val )
            {
                $str[$key] = static::str_ireplace($search, $replace, $val, $count);
            }
            return $str;
        }
        ...
        if ( \is_array($search) )
        {
            ...
            foreach ( $keys as $k )
            {
                if ( \is_array($replace) )
                {
                    ...
                    {
                        $str = static::str_ireplace($search[$k], $replace[$k], $str, $count);
                    }
                    else
                    {
                        $str = static::str_ireplace($search[$k], '', $str, $count);
                    }
                }
                else
                {
                    $str = static::str_ireplace($search[$k], $replace, $str, $count);
                }
            }
            return $str;
        }

        $search = static::strtolower($search);
        $str_lower = static::strtolower($str);
    ...
    {
        if ( static::is_ascii($str) && static::is_ascii($search) ) return \stristr($str, $search);

        if ( $search == '' ) return $str;

        $str_lower = static::strtolower($str);
        $search_lower = static::strtolower($search);
    ...
    public static function strspn($str, $mask, $offset = null, $length = null)
    {
        ...
        if ( static::is_ascii($str) && static::is_ascii($mask) ) return ($offset === null) ? \strspn($str, $mask) : (($length === null) ? \strspn($str, $mask, $offset) : \strspn($str, $mask, $offset, $length));

        if ( $offset !== null || $length !== null )
        {
            $str = static::substr($str, $offset, $length);
        }
        ...
        return isset($matches[0]) ? static::strlen($matches[0]) : 0;
    }
    ...
    public static function strcspn($str, $mask, $offset = null, $length = null)
    {
        ...
        if ( static::is_ascii($str) && static::is_ascii($mask) ) return ($offset === null) ? \strcspn($str, $mask) : (($length === null) ? \strcspn($str, $mask, $offset) : \strcspn($str, $mask, $offset, $length));
    ...
    {
        if ( static::is_ascii($str) && static::is_ascii($pad_str) ) return \str_pad($str, $final_str_length, $pad_str, $pad_type);

        $str_length = static::strlen($str);

        if ( $final_str_length <= 0 || $final_str_length <= $str_length ) return $str;

        $pad_str_length = static::strlen($pad_str);
        $pad_length = $final_str_length - $str_length;

        if ( $pad_type == \STR_PAD_RIGHT )
        {
            $repeat = \ceil($pad_length / $pad_str_length);
            return static::substr($str . \str_repeat($pad_str, $repeat), 0, $final_str_length);
        }

        if ( $pad_type == \STR_PAD_LEFT )
        {
            $repeat = \ceil($pad_length / $pad_str_length);
            return static::substr(\str_repeat($pad_str, $repeat), 0, \floor($pad_length)) . $str;
        }
        ...
        if ( $pad_type == \STR_PAD_BOTH )
        {
            ...
            $pad_left = static::substr(\str_repeat($pad_str, $repeat_left), 0, $pad_length_left);
            $pad_right = static::substr(\str_repeat($pad_str, $repeat_right), 0, $pad_length_left);

            return $pad_left . $str . $pad_right;
        }

        \trigger_error('utf8::str_pad: Unknown padding type (' . $pad_type . ')', \E_USER_ERROR);
    }
    ...
    public static function str_split($str, $split_length = 1)
    {
        ...
        if ( static::is_ascii($str) ) return \str_split($str, $split_length);

        if ( $split_length < 1 ) return false;

        if ( static::strlen($str) <= $split_length ) return array($str);
    ...
    {
        if ( static::is_ascii($str) ) return \strrev($str);
    ...
    {
        if ( $charlist === null ) return \trim($str);
    ...
    public static function ltrim($str, $charlist = null)
    {
        ...
        if ( static::is_ascii($charlist) ) return \ltrim($str, $charlist);
    ...
    public static function rtrim($str, $charlist = null)
    {
        ...
        if ( static::is_ascii($charlist) ) return \rtrim($str, $charlist);
        ...
        for( $i = 0; $i < $len; $i ++ )
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    \trigger_error('utf8::to_unicode: Incomplete multi-octet sequence in UTF-8 at byte ' . $i, \E_USER_WARNING);
                    return false;
                }
            }
        }
    ...
    public static function from_unicode($arr)
    {
        ...
        foreach ( $keys as $k )
        {
            ...
            {
                // Found a surrogate
                \trigger_error('utf8::from_unicode: Illegal surrogate at index: ' . $k . ', value: ' . $arr[$k], \E_USER_WARNING);
                return false;
            }
            // 3 byte sequence
            elseif ( $arr[$k] <= 0xffff )
            ...
            {
                \trigger_error('utf8::from_unicode: Codepoint out of Unicode range at index: ' . $k . ', value: ' . $arr[$k], \E_USER_WARNING);
                return false;
            }
        }
