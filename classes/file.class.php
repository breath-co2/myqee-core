...
class File
{
    ...
    {
        $info = static::check_and_get_path($file);

        if ( static::can_do_run($storage) )
        {
            # 系统内部运行时或小于等于1台服时执行
        ...
        {
            return static::call_http_host($storage ,'file/create_file', $info[0], $info[1], $data , $flags , $context);
        }
    }
    ...
    {
        $info = static::check_and_get_path($dir);
        ...
        {
            return static::call_http_host($storage,'file/create_dir',$info[0], $info[1], $auto_create_default_file);
        }
    }
     ...
     * @return boolean
     */
    public static function unlink($file, $storage=null)
    {
        $info = static::check_and_get_path($file);
        ...
        {
            return static::call_http_host($storage,'file/unlink',$info[0], $info[1]);
        }
    }
    ...
    public static function remove_dir($dir , $storage = 'default')
    {
        ...
        if ( static::can_do_run($storage) )
        {
            ...
            while ( ($file = \readdir($handle)) !== false )
            {
                ...
                {
                    $tmp_dir = $dir . \DS . $file;
                    \is_dir($tmp_dir) ? static::remove_dir($tmp_dir) : @\unlink($tmp_dir);
                }
            }
            \closedir($handle);
        ...
        {
            return static::call_http_host($storage,'file/remove_dir',$info[0], $info[1] );
        }
    }
    ...
    public static function move_dir($fromdir, $todir, $autocoverageold = true , $storage = 'default')
    {
        ...
        $info1 = static::check_and_get_path($fromdir);
        $info2 = static::check_and_get_path($todir);
        ...
        if ( static::can_do_run($storage) )
        {
            ...
            {
                # 创建目标目录
                static::create_dir($todir,false,$storage);
            }
            ...
            foreach ( $files as $file )
            {
                ...
                if ( \is_dir($file) )
                {
                    ...
                    # 移动目录
                    $donum2 = static::move_dir($file, $tofile, $autocoverageold, $storage);
                    if ($donum2)
                    {
                        $donum[0] += $donum2[0];
                        $donum[1] += $donum2[1];
        ...
        {
            return static::call_http_host($storage,'file/move_dir', $info1[0], $info1[1], $info2[0], $info2[1], $autocoverageold);
        }
    }
    ...
    public static function copy_dir($fromdir, $todir, $autocoverageold = true , $storage = 'default')
    {
        ...
        if ( static::can_do_run($storage) )
        {
            ...
            foreach ( $files as $file )
            {
                ...
                if ( \is_dir($file) )
                {
                    ...
                    # 移动目录
                    $donum2 = static::copy_dir($file, $tofile, $autocoverageold, $storage);
                    if ($donum2)
                    {
                        $donum[0] += $donum2[0];
                        $donum[1] += $donum2[1];
        ...
        {
            return static::call_http_host($storage,'file/copy_dir', $info1[0], $info1[1], $info2[0], $info2[1], $autocoverageold);
        }
    }
    ...
    {
        $info = static::check_and_get_path($filename);
        ...
        {
            return static::call_http_host($storage,'file/split',$info[0], $info[1], $piece_size);
        }
    }
    ...
    public static function join($filename , $storage = 'default')
    {
        ...
        {
            return static::call_http_host($storage,'file/join', $info[0], $info[1]);
        }
    }
    ...
    {
        return isset(static::$mimes[$extension])?static::$mimes[$extension][0]:false;
    }
    ...
    {
        return isset(static::$mimes[$extension])?((array)static::$mimes[$extension]):array();
    }
    ...
    {
        return (\IS_SYSTEM_MODE || 'none' == static::sync_mode() ||\count(\Core::config('core.web_server_list.' . $storage)) <= 1) ? true : false;
    }
